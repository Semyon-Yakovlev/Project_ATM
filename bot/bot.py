...
async def cmd_history(message: Message):
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(f"{url}/history/{user_id}") as res:
            history = await res.json()

    if len(history) != 0:
        msg = "\n".join(
            [
                f"({pred['lat']}, {pred['long']}, {pred['atm_group']}): {pred['prediction']}"
                for pred in history
            ]
        )
...
async def cmd_predict_batch(message: Message, state: FSMContext):
    ...
    await message.answer(
        "Введите координаты и наименование банка (информация о каждом банкомате с новой строки)",
        reply_markup=cancel(),
    )
...
async def send_batch(message: Message, state: FSMContext):
    ...
    try:
        ...
        data = [
            list(map(float, row.split(", ")[:2])) + [row.split(", ")[2]] for row in rows
        ]
