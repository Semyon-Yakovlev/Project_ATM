...
from . import data_pipeline_dir, model_dir, test_dir, train_dir
...
def train(cfg: DictConfig):
    ...
    with start_run():
        ...
        with open(data_pipeline_dir, "rb") as file:
            data_pipeline = pickle.load(file)
        train_data = data_pipeline.transform(
            transform_data(read_csv(train_dir, sep=";"))
        )
        test_data = data_pipeline.transform(transform_data(read_csv(test_dir, sep=";")))
