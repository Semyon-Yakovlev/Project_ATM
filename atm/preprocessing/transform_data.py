...
settlements = pd.read_csv("./atmFastapi/data/settlements_processed.csv", sep=";")
salary = pd.read_csv("./atmFastapi/data/salary_processed.csv", sep=";")
...
def transform_data(data):
    ...
    for category in [
        ...
        lat_long = []
        with open(f"./atmFastapi/data/osm_node_{category}.json", encoding="utf8") as f:
        ...
        cross_merge[f"n_{category}_100"] = (cross_merge["distance"] <= 0.3).astype(
            np.uint8
        )
        cross_merge[f"n_{category}_300"] = (cross_merge["distance"] <= 0.1).astype(
            np.uint8
        )
        data = data.merge(
            cross_merge.groupby("atm_id")
            .aggregate({f"n_{category}_100": "sum", f"n_{category}_300": "sum"})
            .reset_index(),
            on="atm_id",
            how="left",
        )
