...
def calc_dist(lat_1, long_1, lat_2, long_2):
    ...
    distance = 6378.137 * 2 * np.arcsin(np.sqrt(np.sin(lat_diff/2.0)**2 + np.cos(lat_1) * np.cos(lat_2) * np.sin(long_diff/2.0)**2))
    return distance


def find_population(lat, long, set_data):
    distances = set_data.apply(
        lambda settlement: calc_dist(lat, long, settlement['latitude_rad'], settlement['longitude_rad']),
        axis=1)
...
def add_features():
    ...
    for category in [
        ...
        set_data = settlements[['region', 'settlement', 'latitude_dd', 'longitude_dd', 'population']].groupby(by=['region', 'settlement']).aggregate({'latitude_dd':'mean',
                                                                                                          'longitude_dd':'mean',
                                                                                                          'population':'sum'}).reset_index()
        set_data[['latitude_rad', 'longitude_rad']] = np.radians(set_data[['latitude_dd', 'longitude_dd']])
        ...
        train.drop(columns=['atm_id', 'id', 'atm_group', 'address', 'address_rus', 'lat_rad', 'long_rad', 'key', 'region'], inplace=True)
